...
## 9. Статус исполнения — ЗАВЕРШЕНО (v1)

Все этапы выполнены и запушены в `origin/main`:

- **Этап 0** — токены light + примитивы (`PALETTE = LIGHT`, `card_shadow`, `WorkspaceCard`).
- **Этап 1** — оболочка: плавающий sidebar-плитка, тулбар со слотом контролов, `SegmentedControl`.
- **Этап 2** — карточка Valuation: `ScrollableCard` + `KeyValueGrid`, таблицы платежей/кривой, фикс-футер.
- **Этап 3** — карточка Parameters: 2-колоночная сетка, полноширинные поля, скролл-при-переполнении.
- **Этап 4** — состояния: idle, валидация ввода с подсветкой, stale-баннер, статус-цвет цены.
- **Этап 5** — раскатка на 6 воркспейсов: снят дубль заголовка, панели/KPI с тенью (общие правки).
- **Этап 6** — чистка: проверено отсутствие «зашитых» тёмных цветов в мигрированных файлах
  (`ui/*`, `app/panels/pricing_*`, `app/main_window`); финальный прогон.

**Решение по легаси:** 33 одно-инструментальные панели (`app/panels/*_panel.py` кроме
dashboard/portfolio) и тёмный `app/styles.py` — мёртвый код, недостижимый из воркстейшна
и не покрытый тестами; вытеснены хабом Pricing. Помечены **deprecated** (см. docstring в
`app/styles.py`), не ре-темизированы и не удалены (удаление — отдельное решение пользователя).

**Будущее (вне v1):** рантайм-переключение темы (`ThemeManager` + `themeChanged`); опционально
удаление/перенос legacy-панелей; визуальный регресс против `design/pricing_v6_light.png`.
